...
% iteration to compute corrections for atmospheric stability    
    ...
    for i = 1:20;          
        ...
        [idx_m_vu,idx_m_u,idx_t_vu,idx_t_u,idx_s,idx_vs] = zeta_indices(zeta);
        
        ustar(idx_m_vu) = (Uz(idx).*const_vonKarman)./...
            ((log((zetam.*obu(idx))./zo(idx)) - psi_zeng(1,zetam)) + ...
            1.14.*(((-zeta(idx)).^0.333) - ((-zetam).^0.333))); 
        ustar(idx_m_u) = (Uz(idx).*const_vonKarman)./...
            (log(hu./zo(idx)) - psi_zeng(1,zeta(idx)));
        ustar(idx_s) = (Uz(idx).*const_vonKarman)./...
            (log(hu./zo(idx)) + 5.*zeta(idx));
        ustar(idx_vs) = (Uz(idx).*const_vonKarman)./...
        ...
        [~,~,idx_t_vu,idx_t_u,idx_s,idx_vs] = zeta_indices(zeta);
        
        tstar(idx_t_vu) = (const_vonKarman.*(ta(idx) - ts(idx)))./...
            ((log((zetat.*obu(idx))./zot(idx)) - psi_zeng(2,zetat)) + ...
            0.8.*((-zetat).^-0.333 - ((-zeta(idx))).^-0.333));
        tstar(idx_t_u) = (const_vonKarman.*(ta(idx) - ts(idx)))./...
            (log(ht./zot(idx)) - psi_zeng(2,zeta(idx)));
        tstar(idx_s) = (const_vonKarman.*(ta(idx) - ts(idx)))./...
            (log(ht./zot(idx)) + 5.*zeta(idx));
        tstar(idx_vs) = (const_vonKarman.*(ta(idx) - ts(idx)))./...
        ...
        qstar(idx_t_vu) = (const_vonKarman.*(q_z(idx) - q_s(idx)))./...
            ((log((zetat.*obu(idx))./zoq(idx)) - psi_zeng(2,zetat)) + ...
            0.8.*((-zetat).^-0.333 - ((-zeta(idx))).^-0.333));
        qstar(idx_t_u) = (const_vonKarman.*(q_z(idx) - q_s(idx)))./...
            (log(hq./zoq(idx)) - psi_zeng(2,zeta(idx)));
        idx = zeta >= 0 & zeta <= 1; % stable conditions
        qstar(idx_s) = (const_vonKarman.*(q_z(idx) - q_s(idx)))./...
            (log(hq./zoq(idx)) + 5.*zeta(idx));
        idx = zeta > 1; % very stable conditions
        qstar(idx_vs) = (const_vonKarman.*(q_z(idx) - q_s(idx)))./...
        ...
        u10(idx_m_vu) = (ustar(idx)./const_vonKarman).*...
            ((log((zetam.*obu(idx))./zo(idx)) - psi_zeng(1,zetam)) + ...
            1.14.*(((-zeta(idx)).^0.333) - ((-zetam).^0.333)));
        u10(idx_m_u) = (ustar(idx)./const_vonKarman).*...
            (log(10./zo(idx)) - psi_zeng(1,zeta(idx)));
        u10(idx_s) = (ustar(idx)./const_vonKarman).*...
            (log(10./zo(idx)) + 5.*zeta(idx));
        u10(idx_vs) = (ustar(idx)./const_vonKarman).*...
            ((log(obu(idx)./zo(idx)) + 5) + (5.*log(zeta(idx)) + zeta(idx) - 1));
        
        % calcuate air temperature at 10 m
        t10(idx_t_vu) = ((tstar(idx)./const_vonKarman).*...
            ((log((zetat.*obu(idx))./zot(idx)) - psi_zeng(2,zetat)) + ...
            0.8.*((-zetat).^-0.333 - ((-zeta(idx))).^-0.333))) + ts(idx);
        t10(idx_t_u) = ((tstar(idx)./const_vonKarman).*...
            log(10./zot(idx)) - psi_zeng(2,zeta(idx))) + ts(idx); 
        idx = zeta >= 0 & zeta <= 1;% stable conditions
        t10(idx_s) = ((tstar(idx)./const_vonKarman).*...
            (log(10./zot(idx)) + 5.*zeta(idx))) + ts(idx); 
        idx = zeta > 1; % very stable conditions
        t10(idx_vs) = ((tstar(idx)./const_vonKarman).*...
            ((log(obu(idx)./zot(idx)) + 5) + (5.*log(zeta(idx)) + zeta(idx) - 1))) + ts(idx);
        
        % calcuate specific humidity at 10 m
        q10(idx_t_vu) = ((qstar(idx)./const_vonKarman).*...
            ((log((zetat.*obu(idx))./zoq(idx)) - psi_zeng(2,zetat)) + ...
            0.8.*((-zetat).^-0.333 - ((-zeta(idx))).^-0.333))) + q_s(idx);            
        q10(idx_t_u) = ((qstar(idx)./const_vonKarman).*...
            (log(10./zoq(idx)) - psi_zeng(2,zeta(idx)))) + q_s(idx);
        q10(idx_s) = ((qstar(idx)./const_vonKarman).*...
            (log(10./zoq(idx)) + 5.*zeta(idx))) + q_s(idx);
        q10(idx_vs) = ((qstar(idx)./const_vonKarman).*...
...
function [idx_m_vu,idx_m_u,idx_t_vu,idx_t_u,idx_s,idx_vs] = zeta_indices(zeta)
