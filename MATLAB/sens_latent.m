...
% define constants
    ...
    const_Gravity = 9.780310.*(1+(0.00530239.*(sin(abs(lat)).^2) - 0.00000587.*(sin(abs(2*lat)).^2) - (31.55e-8)*alt)); % gravitational acceleration, m/s2
...
% iteration to compute corrections for atmospheric stability    
    ...
    for i = 1:20;          
        ...
        q10(idx_vs) = ((qstar(idx_vs)./const_vonKarman).*...
            ((log(obu(idx_vs)./zoq(idx_vs)) + 5) + (5.*log(zeta(idx_vs)) + ...
